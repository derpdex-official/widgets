import {
  ...
  PERMIT2_ADDRESS_BY_CHAINS,
  PermitSingle,
} from '@derpdex/permit2-sdk'
...
export function usePermitAllowance(token?: Token, owner?: string, spender?: string) {
  ...
  const contract = useContract<Permit2>(PERMIT2_ADDRESS_BY_CHAINS[chainId ?? 280], PERMIT2_ABI)
      ...
      WidgetPromise.from(
        async () => {
          ...
          const { domain, types, values } = AllowanceTransfer.getPermitData(
            permit,
            PERMIT2_ADDRESS_BY_CHAINS[chainId],
            chainId
          )
